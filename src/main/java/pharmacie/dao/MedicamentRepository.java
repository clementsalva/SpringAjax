...
public interface MedicamentRepository extends JpaRepository<Medicament, Integer> {
    /**
     * Calcule le nombre d'unités vendues pour chaque médicament d'une catégorie
     * donnée.
     *
    ...
    @Query("""
                SELECT m FROM Medicament m
                WHERE m.indisponible = false
                  AND (m.unitesEnStock - m.unitesCommandees) < m.niveauDeReappro
            """)
    ...
    @Query("""
                SELECT m FROM Medicament m
                WHERE m.indisponible = false
                  AND m.unitesEnStock < 10
            """)
    List<Medicament> medicamentsStockBas();
